...
def fetch_result(htno, url=None):
    """Fetch a single hallticket from `url` (if provided) or default `URL`.
    Returns the same dict as before or None when result not found.
    """
    ...
    # use a copy of headers so we can set Referer safely
    headers = dict(HEADERS)
    headers["Referer"] = target_url

    response = session.post(
        target_url, data=payload, headers=headers, timeout=15, verify=False
    )
...
def run_job(url, start_ht, end_ht, output_path, protect_password=None, max_workers=10, progress_callback=None):
    """Run a scraping job programmatically and produce an Excel file.

    - `progress_callback(job_id, done_count, total)` can be used to report progress.
    ...
    total = end_ht - start_ht + 1
    done = 0
    items = []
    ndjson_path = Path(output_path).with_suffix('.ndjson')

    lock_local = threading.Lock()

    def _worker(htno):
        nonlocal done
        data = fetch_result(htno, url=url)
        if data:
            with lock_local:
                items.append(data)
            ...
            placeholder = {"student": {"hallticket": str(htno)}, "marks": [], "result": None, "status": "NO_RESULT"}
            with lock_local:
    ...
    # make sure any old ndjson file is removed
    try:
    ...
    hts = [str(ht) for ht in range(int(start_ht), int(end_ht) + 1)]
    with ThreadPoolExecutor(max_workers=max_workers) as ex:
        futures = [ex.submit(_worker, ht) for ht in hts]
        for _ in as_completed(futures):
            pass

    # finally, generate excel using helper
    try:
        from json_to_excel import items_to_excel
    except Exception as e:
        raise RuntimeError(f"Cannot import json_to_excel: {e}")

    outp = Path(output_path)
    items_to_excel(items, outp, protect_password)
    return outp
